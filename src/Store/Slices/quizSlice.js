...
//     .then((data) => data)
// }
// export const loadQuestions = createAsyncThunk("qusetions", getQuestions);
const initialState = {
  questions: [
    {
  ...
  points: 0,
  timer: 1000,
  maxPts:(questions)=>questions.reduce((acc, cur) => acc + cur.points, 0)
};
const quizSlice = createSlice({
  ...
  reducers: {
    ...
    nextQusetion: (state, action) => {
      state.curIndex + 1 < state.questions.length && state.curIndex++;
    },
    ready:(state,action)=>{
      state.status = "ready"
    },
...
export const { nextQusetion, start, addPoints, finish, reduceTimer,resetState,ready } =
  quizSlice.actions;
